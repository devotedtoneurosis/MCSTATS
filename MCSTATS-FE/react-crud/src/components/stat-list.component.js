...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  console.log(payload[0].payload[0].toString());

  if (active && payload && payload.length && 
    typeof payload[0].payload[2].value != 'undefined' &&
    typeof payload[0].payload[3].value != 'undefined') {
    return (
      <div className="custom-tooltip">
        <p className="players">Playerz:{`${label} : ${payload[0].payload[1].value}`}</p>
        <p className="weight">Weight:{`${label} : ${payload[0].payload[2].value}`}</p>
        <p className="url">URL:{`${label} : ${payload[0].payload[3].value}`}</p>
      </div>
    );
  }

  if (active && payload && payload.length && typeof payload[0].payload[1].value != 'undefined') {
    return (
      <div className="custom-tooltip">
        <p className="players">Players:{`${label} : ${payload[0].payload[1].value}`}</p>
      </div>
    );
  }
